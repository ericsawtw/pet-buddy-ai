...
// 驗證請求，回傳錯誤訊息字串；通過則回 null
export function validateAnalyzeRequest(body: AnalyzeRequest): string | null {
  if (!body.symptoms || body.symptoms.trim().length < 2) {
    return "請描述毛孩的症狀（至少 2 個字）";
  }
  if (!body.petType || !["dog", "cat"].includes(body.petType)) {
    return "請選擇寵物類型（狗 或 貓）";
  }
  return null;
}
...
`.trim();
  ...
  if (!textBlock || textBlock.type !== "text") {
    throw new Error("AI 未回傳分析結果，請稍後再試");
  }
