...
import { getUser, consumeFreeUse } from "@/lib/users";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!member || member.freeUsesRemaining <= 0) {
      return Response.json(
        {
          error: "免費次數已用完，付費方案即將推出 🙏",
          outOfFree: true,
        },
        { status: 403 }
      );
...
`.trim();
    ...
    // 記錄到後台（含照片、擁有者）。失敗不影響使用者拿到分析結果 —— 分析功能優先
    try {
    ...
    return Response.json({ ...parsed, freeUsesRemaining });
  } catch (err) {
